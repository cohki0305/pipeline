...
export type MainCommand =
  | { cmd: "run"; issue: number; designDocPath: string | undefined; mode: "resume" | "fresh"; worktreePath: string | undefined }
  | { cmd: "babysit" }
...
export function parseMain(args: string[]): MainCommand {
  ...
  if (first === "planning-agent" || first === "planning") {
    const arg = rest[0];
    if (arg === undefined || arg === "list") return { cmd: "planning-agent", op: "list" };
    const agent = parsePlanningAgentArg(arg);
    return agent ? { cmd: "planning-agent", op: "set", agent } : { cmd: "help" };
  }
  ...
  if (Number.isInteger(issue) && issue > 0) {
    ...
    return {
      ...
      worktreePath: w !== -1 && args[w + 1] ? args[w + 1] : undefined,
    };
  }
  return { cmd: "help" };
}
...
const USAGE = `使い方（.agent-pipeline.json のあるプロジェクトルートで実行）:
  pipeline <issue番号> [--design <設計書パス>] [--fresh] [--worktree <パス>]  issue を実装して PR 作成まで自動で回す（既定は --resume。--worktree で既存 worktree を作業場所に指定）
  ...
  pipeline planning-agent [list | claude | codex]     設計・レビュー担当を切り替え（codex = codexSol）`;
...
export async function main(args: string[]): Promise<number> {
  ...
  switch (command.cmd) {
    ...
    case "run":
      try {
        const { prUrl, reportPath } = await runPipeline(makePipelineDeps(projectRoot), command.issue, {
          ...
          mode: command.mode,
          worktreePath: command.worktreePath ? resolve(command.worktreePath) : undefined,
        });
    ...
    case "planning-agent": {
      ...
      const config = JSON.parse(readFileSync(path, "utf8")) as Record<string, unknown>;
      if (command.op === "list") {
        console.log(`設計・レビュー担当: ${formatPlanningAgent(resolvePlanningAgentSetting(config))}`);
        return 0;
      }
      const next = updatePlanningAgent(config, command.agent);
      writeFileSync(path, `${JSON.stringify(next, null, 2)}\n`);
      console.log(`planningAgent: ${formatPlanningAgent(resolvePlanningAgentSetting(next))}`);
      return 0;
    }
  }
}
